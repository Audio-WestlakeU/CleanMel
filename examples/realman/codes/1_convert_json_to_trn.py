...
LastEditTime: 2025-01-06 11:08:10
FilePath: /InASR/examples/realman/codes/1_convert_json_to_trn.py
'''
import os
import json
import sys
...
default_json_file = os.path.join("./examples/realman/results", 'test_dp_speech.json')
...
for mode in ['static']:
    ...
    for scene in scenes:
        ...
        trn_file = f'{scene_path}/{args.json_file.split("/")[-1][:-5]}_{scene}.trn'

        # read '*.json' file
        with open(args.json_file, 'r') as file:
            data = json.load(file)
        
        from NAME_DICT import name2abbr
        scene_id = name2abbr[scene]
        ...
        with open(trn_file, 'w') as file:
            for item in data:
                ...
                elif scene_id in item[0] and ('CH0' in item[0]) and '_S_' in item[0]:
                    s_spaced = " ".join(item[1])
                    file.write(f"{s_spaced}  (S{item[0].split('_')[-3]}-{item[0].split('_')[-2]}) \n")
